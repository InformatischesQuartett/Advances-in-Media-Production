...
public class InputSelector : MonoBehaviour
{
    ...
    private bool _helpButtonPressed;
	 ...
	 **/
    private readonly bool[] _selectedMode = new bool[5];
    ...
    private void Start()
    {
        ...
        {
            font = (Font) Resources.Load("Fonts/BNKGOTHM"),
            normal = {textColor = Color.white},
            fontSize = (int) (Screen.height*0.09f)
        };
    ...
    private void NewSelection(int mode)
    {
        _currSelection = mode;
    ...
    private void OnGUI()
    {
        ...
        if (_enableGUI)
        {
            ...
            _horizScrollPos = GUILayout.BeginScrollView(_horizScrollPos, false, false,
                GUILayout.Width(Screen.width*0.7f), GUILayout.Height(Screen.height*0.8f));
            ...
            if (GUILayout.Toggle(_selectedMode[0], "Sony, Side by Side"))
            {
                ...
                if (_currSelection != lastSelection)
                {
                    ...
                    AVProLiveCameraManager.Instance.GetDevice(0).Start(5);
                }
            }
            /*Sony Framepacking*/
            if (GUILayout.Toggle(_selectedMode[1], "Sony, Framepacking"))
            {
                ...
                if (_currSelection != lastSelection)
                {
                    if (AVProLiveCameraManager.Instance.NumDevices > 0)
                    {
                        ...
                        AVProLiveCameraManager.Instance.GetDevice(0).Start(19);
                    }
                }
            }
            ...
            if (GUILayout.Toggle(_selectedMode[2], "2x Canon EOS C300"))
            {
                ...
                if (_currSelection != lastSelection)
                {
                    if (AVProLiveCameraManager.Instance.NumDevices > 1)
                    {
                        ...
                        AVProLiveCameraManager.Instance.GetDevice(1).Close();
                        AVProLiveCameraManager.Instance.GetDevice(0).Start(5);
                        AVProLiveCameraManager.Instance.GetDevice(1).Start(5);
                    }
                }
            }
            ...
            if (_currSelection == 0 || _currSelection == 1)
            {
                ...
                {
                    GUILayout.Label(
                        "Caution: There is no camera detected! The Sony camera needs to be in the first HDMI-In slot. Please also make sure it is set to 3D mode.");
                }
            }
            ...
            if (_currSelection == 2)
            {
                ...
                {
                    GUILayout.Label(
                        "Caution: There are no two cameras connected! The Input has to be SDI or maybe you have chosen the wrong mode.");
                }
            }
            ...
            {
                GUILayout.Label(
                    "If there is no camera image to see, please make sure the Sony camera is plugged in to the first HDMI-In slot and it is set to 3D mode with the corresponding 3D setting. For other problem solving solutions see the instructions below.\n");
            }
            if (_currSelection == 2)
            {
                GUILayout.Label(
                    "If there is no camera image to see, please make sure that both camera are plugged in to the right SDI-In slot. See the documentation for further information to this topic.For other problem solving solutions see the instructions below.\n");
            }

            GUILayout.Label(
                "If there are no cameras detected there could be a problem with the Blackmagic options. You need to configure them in the Control Center.\n\n");
            if (GUILayout.Button("Open Blackmagic Control Center"))
            {
                ...
                {
                    Application.OpenURL(
                        @"C:\Program Files (x86)\Blackmagic Design\Blackmagic Desktop Video\desktopcp.exe");
                }
                catch (UnityException e)
                {
                    Debug.Log("There is no program like this installed: " + e.Message);
                }
            }
            GUILayout.EndVertical();
            ...
            GUI.Label(new Rect(Screen.width*0.02f, Screen.height/1.2f, Screen.width, Screen.height*0.10f),
                "Input Selector", _fontStyle);
            //Help Button
            ...
            {
                GUI.Label(new Rect(Screen.width/1.6f, Screen.height*0.10f, Screen.width*0.30f, Screen.height*0.8f),
                    "This is Cyclops - a S3D camera viewfinder for composition on set. You'll need a Sony3D or two Canon EOS C300 cameras plus the Oculus Rift DK2. Please read the documentation for further information.\n Application created by Fabian Gaertner, Sarah Haefele, Alexander Scheurer and Linda Schey for the subject Advanced Media Production at Hochschule Furtwangen in January 2015.");
            }
        }
        else
        {
            GUI.Label(new Rect(Screen.width*0.02f, Screen.height/1.2f, Screen.width, Screen.height*0.10f),
                "Please put on your HMD now.", _fontStyle);
        }
